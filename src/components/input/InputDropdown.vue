<template>
    <select class="bg-white focus:text-black border-gray-300 focus:border-blue-500 outline-none block w-full pl-3 pr-10 py-2 transition duration-100 ease-in-out h-12
        ...
        @change="setData($event)" :name="id" :disabled="disabled" :class="{ 'text-gray-400': !selected || selected.length < 1, 'text-black': selected.length > 0 }">
...
const props = defineProps(["selected", "optionList", "id", "disabled", "placeholder"])
